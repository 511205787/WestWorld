# trajworld_runtime/wrapper_trajworld.py
import os
...
from .trajworld import TrajWorld, symlog_torch, symexp_torch
from .trajworld_utils import transform, transform_from_probs
...
class WorldModelWrapper:
    def __init__(self, ckpt_path: str, cfg_yaml: str, device: str = "cuda",
        ...
        self.model = TrajWorld(self.cfg).to(self.device)
    ...
    def _ensure_cfg_layout(self) -> None:
        if "method" not in self.cfg:
            ...
            for key in (
                ...
                "rel_sigma", "mask_ratio", "use_symlog", "lr", "weight_decay",
    ...
    def rollout_batch_once(self, s0_mppi: np.ndarray, actions_mppi: np.ndarray) -> np.ndarray:
        ...
        support, sigma, c = self.model._get_support_sigma(Do, Da, self.device)
        if self.model.use_symlog:
            obs_train = symlog_torch(obs_t, c)
            act_train = symlog_torch(act_t, c)
        else:
            obs_train = obs_t
            act_train = act_t

        cur_token = torch.cat([obs_train, act_train[:, 0, :]], dim=-1).unsqueeze(1)

        variate_mask = torch.cat([self.obs_mask_base, self.act_mask_base], dim=-1)
        variate_mask = variate_mask.unsqueeze(0).repeat(B, 1)

        caches = self.model.model.get_empty_cache(batch_size=B * M, device=self.device)
        debug_kv_check = os.environ.get("TRAJWORLD_KV_CHECK") == "1"
        debug_max_steps = int(os.environ.get("TRAJWORLD_KV_CHECK_STEPS", "2"))
        debug_tokens = [] if debug_kv_check else None
        pred_obs_list = []
        for step in range(H):
            inputs_probs = transform("gauss", cur_token, support, sigma)
            ...
            # --- BEGIN KV-CACHE CHECK (remove after debugging) ---
            if debug_kv_check and step < debug_max_steps:
                debug_tokens.append(cur_token)
                hist = torch.cat(debug_tokens, dim=1)
                L = hist.shape[1]
                inputs_ref = transform("gauss", hist, support, sigma)
                obs_act_ref = torch.zeros((B, L, M), device=self.device, dtype=torch.long)
                if Da > 0:
                    obs_act_ref[:, :, Do:] = 1
                padding_ref = torch.ones((B, L), device=self.device)
                logits_ref = self.model.model.call_variate_mask(
                    inputs_ref, obs_act_ref, padding_ref, variate_mask, training=False
                )
                diff = (logits_ref[:, -1] - logits[:, -1]).abs().max().item()
                # if diff > 1e-4:
                print(f"[KV-CHECK] step={step} max_diff={diff:.3e}")
            # --- END KV-CACHE CHECK ---
            probs = torch.softmax(logits, dim=-1)
            pred_vals_train = transform_from_probs(probs, support)
            next_pred_train = pred_vals_train[:, -1, :]

            pred_obs_train = next_pred_train[:, :Do]
            if self.model.use_symlog:
                pred_obs_norm = symexp_torch(pred_obs_train, c)
            else:
                pred_obs_norm = pred_obs_train
            ...
            if step < H - 1:
                act_next = act_train[:, step + 1, :]
                next_token = torch.cat([pred_obs_train, act_next], dim=-1)
                cur_token = next_token.unsqueeze(1)
