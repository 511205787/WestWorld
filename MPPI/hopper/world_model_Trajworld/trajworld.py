...
from .base_model_multichannel import BaseModel
...
class Block(nn.Module):
    def __init__(self, h_dim: int, max_T: int, n_heads: int, drop_p: float):
        ...
        # two FFN blocks as in the original code
...
class TrajWorldTransformer(nn.Module):
    def __init__(
        ...
        self.embed = nn.Embedding(vocab_size, h_dim)           # (unused, but keep the interface)
        self.embed_obs_act = nn.Embedding(2, h_dim)             # 0 obs/reward, 1 action
        self.embed_timestep = nn.Embedding(max_timestep, h_dim) # time step
    ...
    def _apply_input_embeds(
        ...
        # random masking (variate / time-step mask)
        ...
        # prompt time steps run from 0..T-1; kv-cache is handled separately
        embedded = embedded + self.embed_timestep(timesteps)[:, None, :]

        # variable embedding
        if self.use_variate_embed:
            if self.shuffle_variate:
                # shuffle independently within each batch
                # generate the (B, M) index matrix
                perms = []
                for _ in range(B):
    ...
    def call_kv_cache(
        ...
        # time-step embeddingfrom
        t0 = caches[0][0].shape[2]  # 
...
# ---------- Build shared-group boundaries from stats ----------
...
def build_shared_support_from_stats(
    ...
    """
    Returns:
      support (M, K+1): one set of boundaries for obs / rew / act, concatenated as [obs, rew, act]
      sigma   (M,)
      c       float (symlog constant; if use_symlog=False, stats['c'] is still returned but not used)
    """
    M = obs_dim + 1 + act_dim
...
# ============================================================
# LightningModule: follow the JAX pipeline for next-token prediction (obs+reward)
# ============================================================
class TrajWorld(BaseModel):
    """
    multi-channel next-token (obs+reward do; action items).
    - construct history = [obs, reward, action]
    - (optional)symlog
    - Input discretization: Gaussian soft labels
    - Target discretization: Uniform one-hot
    - model outputs logits: (B,T,M,K)
    - perform next-token prediction only on obs+reward dims: pred[:, :-1, :Do+1] vs target[:, 1:, :Do+1]
    ...
    def __init__(self, config):
        ...
        # ---- load collected statistics (prewritten by the Dataset into h5_dir/minmax_values.npz)----
        ...
        #     raise FileNotFoundError(f"to {bounds_path}, firstdata sidewritethatfile")
        ...
        #  support/sigma(will only use the 0-1 support)
        self._support_cache = {}
    ...
    def _denorm_obs_to_physical(self, y01_obs: torch.Tensor, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        y01_obs: [B, T-1, Do] —— prediction in the 0-1 normalized space
        Returns: [B, T-1, Do] -- original physical units (inverse normalization + symexp)
        """
        device = y01_obs.device
        # task_id for each trajectory item (assumed constant within a trajectory item)
        task_first = batch["task"][:, 0].long().to(device)  # [B]
        # map to the row indices of the per-task bounds
        rows = torch.tensor([self.taskid2row[int(t.item())] for t in task_first], device=device, dtype=torch.long)

        # take the obs_min/max corresponding to each task in the batch
        mn = self.obs_min[rows]  # [B, Do_max]
        mx = self.obs_max[rows]  # [B, Do_max]

        # align the Do of the current tensor (sometimes it is MAX_OBS_DIM)
        Do = y01_obs.shape[-1]
        mn = mn[:, None, :Do]    # [B,1,Do]
        mx = mx[:, None, :Do]    # [B,1,Do]

        # first map from [0,1] -> symlog space
        y_sym = y01_obs * (mx - mn) + mn
        # then map from symlog -> original physical units
    ...
    def forward(self, batch):
        ...
        # indicator: 0 -> obs+reward, 1 -> action (action is input only)
        ...
        # next-token align(no burn-in)
        # logits_y  = logits[:, :-1, :Do+1, :]         # t -> prediction t+1
        # targets_y = targets_probs[:, 1:, :Do+1, :]    # target time step t+1
        logits_y  = logits[:, :-1, :Do, :]         # t -> prediction t+1
        targets_y = targets_probs[:, 1:, :Do, :]    # target time step t+1
        ...
        # variable weighting (optional): normalize by interval width
        ...
        val_pred = transform_from_probs(probs, support)           # [B,T,M]  (training space)
        val_pred_obs = val_pred[:, :-1, :Do]                      # [B,T-1,Do]
        if self.use_symlog:
            val_pred_obs = symexp_torch(val_pred_obs, c)          # map back to the original space
...
if __name__ == "__main__":
    ...
    padding_mask = torch.ones(B, T, device=device)   # keep all
    ...
    # 3) kv-cache path (following the JAX logic with batch_size=B*M)
    model.eval()
    ...
    # 4) run the model with prompt once
    prompt_T = 2
    ...
    print("[prompt forward] logits:", out_p.shape)  # this should be (B, T, M, vocab)
