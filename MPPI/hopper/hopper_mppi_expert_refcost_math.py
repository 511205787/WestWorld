#!/usr/bin/env python
# -*- coding: utf-8 -*-
# hopper/hopper_mppi_expert_refcost_math.py
"""
...
try:
    from world_model_westworld.wrapper_westworld import WorldModelWrapper
except ImportError:
    import sys
...
def load_world_model(
    ...
    cfg_path = cfg_yaml
    tmp_yaml = None
    if minmax_path is not None:
        cfg = OmegaConf.load(cfg_yaml)
        cfg.minmax_dir = minmax_path
...
def hopper_reward_from_obs(obs, action):
    """
    World-model rollout use: 
    - forward_reward use qvel[0] (rootx velocity) match the Gymnasium Hopper convention
    - terminated use the health criterion to reproduce hopper_is_healthy
    """
    qpos, qvel = _obs_to_qpos_qvel(obs)
...
def rollout_costs_with_world_model(
    ...
    """
    action_sequences_clipped: [P, H, A]  (clipping is recommended to avoid OOD inputs)
    returns costs: [P, H]
    """
    P, H, _ = action_sequences_clipped.shape
    s_prev = np.repeat(np.asarray(s0, dtype=np.float32)[None, :], P, axis=0)
    acts = np.asarray(action_sequences_clipped, dtype=np.float32)

    preds_full = wm.rollout_batch_once(s_prev, acts)  # [P, H, max_obs_dim]
    obs_pred = preds_full[:, :, wm.obs_take_idx][:, :, : wm.mppi_obs_dim]  # [P,H,mppi_obs_dim]

    costs = np.zeros((P, H), dtype=np.float32)
    terminated_flags = np.zeros((P,), dtype=bool)
    done_t = np.full((P,), H, dtype=np.int32)

    for t in range(H):
        s_cur = obs_pred[:, t]
        rew, terminated = hopper_reward_from_obs(s_cur, acts[:, t])
        costs[:, t] = -rew

        newly_term = (~terminated_flags) & terminated
        terminated_flags |= terminated
        done_t[newly_term] = t

    # stay consistent with simulator rollouts: terminate at the end, apply terminal penalty to the remaining horizon, and fill flat
    for i in range(P):
        if terminated_flags[i]:
            remain = H - (done_t[i] + 1)
            if remain > 0:
...
def run_mppi(cfg_dict):
    ...
    if use_world_model:
        wm = load_world_model(
            ...
            obs_take_idx=None,
            act_take_idx=None,
            minmax_path=str(cfg.get("wm_minmax", DEFAULT_WM_MINMAX)),
        )
