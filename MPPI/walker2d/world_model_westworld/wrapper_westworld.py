...
class WorldModelWrapper:
    ...
    #     s0_full = self._embed_obs(s0_mppi)               # [B,78]
    #     actions_full = self._embed_actions(actions_mppi) # [B,H,21]

    #     # === group batch (T = H)===
    #     T = H; Do = self.max_obs_dim; Da = self.max_act_dim
    #     obs = np.zeros((B, T, Do), dtype=np.float32)
    #     act = np.zeros((B, T, Da), dtype=np.float32)
    #     obs[:, 0, :]  = s0_full
    #     act[:, :H, :] = actions_full

    #     # ---- channel mask: all ones along the time dimension; only valid channels are 1 ----
    #     obs_mask = np.zeros((B, T, Do), dtype=np.float32)
    #     obs_mask[:, :, self.obs_take_idx] = 1.0
    #     action_mask = np.zeros((B, T, Da), dtype=np.float32)
    #     action_mask[:, :, self.act_take_idx] = 1.0

    #     # ---- convert to torch and apply 0-1 normalization (unused channels are forced to 0)----
    #     obs_t = torch.from_numpy(obs).to(self.device)
    #     act_t = torch.from_numpy(act).to(self.device)
    #     obs_t = self._norm_obs(obs_t)     # [B,T,78]  —— the input to the model is normalized
    #     act_t = self._norm_act(act_t)     # [B,T,21]

    #     batch = {
    #         "obs":          obs_t,
    #         "action":       act_t,
    #         "obs_mask":     torch.from_numpy(obs_mask).to(self.device),
    #         "action_mask":  torch.from_numpy(action_mask).to(self.device),
    #         "task":         torch.full((B, T), self.task_id, dtype=torch.long, device=self.device),
    #     }

    #     # the model output is an observation prediction in normalized space (length H)
    #     pred_norm, _ = self.model(batch)           # [B,H,78]
    #     pred_norm = torch.clamp(pred_norm, 0.0, 1.0)
    #     pred_raw = self._denorm_obs(pred_norm)     # inverse-normalize back to physical values
    #     # set unused channels back to 0 (to keep the downstream interface consistent)
