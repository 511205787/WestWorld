...
class WestWorld(BaseModel):
    ...
    def __init__(self, config):
        ...
        # ---- hyperparameters ----
        self.K            = int(getattr(self.cfg, "uniform_bins", 256))
        self.h_dim        = int(getattr(self.cfg, "h_dim", 256))
        self.n_blocks     = int(getattr(self.cfg, "n_blocks", 6))
        self.n_heads      = int(getattr(self.cfg, "n_heads", 4))
        self.drop_p       = float(getattr(self.cfg, "drop_p", 0.1))
        self.max_timestep = int(getattr(self.cfg, "max_timestep", 1024))
        self.use_symlog   = bool(getattr(self.cfg, "use_symlog", False)) # may clean
        self.rel_sigma    = float(getattr(self.cfg, "rel_sigma", 0.75))
        self.mask_ratio   = float(getattr(self.cfg, "mask_ratio", 0.0)) # may clean

        # Initialize positional embeddings for observations and actions
        # ---------------------------------------------------------------
        # structural PE preprocessing: read the YAML once to construct act/obs PE dictionaries
        # ---------------------------------------------------------------
        base_dir = Path(__file__).resolve().parent
        SUMMARY_YAML  = base_dir / "robotics_structure_summary.yaml"
        TASKSPEC_YAML = base_dir / "general_task_specific.yaml"

        summary = yaml.safe_load(open(SUMMARY_YAML,  "r"))
        spec    = yaml.safe_load(open(TASKSPEC_YAML, "r"))
        TASK_SPEC   = spec["tasks"]

        # ---------- ① global row limits for collected statistics (obj_max / node_max) ----------
        obj_max  = 0
        node_max = 0
        ...
        mamba_cfg = getattr(self.cfg, "mamba_cfg", None)
    ...
    def forward(self, batch):
        ...
        time_mask[:, prefix_T:] = 0  # the tail is unobserved
        # obs[:, prefix_T:, :] = 0.0  # set observations in the tail to 0
        # ******************************************************************
        #
        ...
        val_pred_obs = val_pred[:, :, :Do]                      # [B,T,Do]
        if self.use_symlog:
