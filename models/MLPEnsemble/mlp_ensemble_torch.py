...
import numpy as np
from models.base_model.base_model_mlpensemble import BaseModel
import wandb
...
class MLPEnsemble(BaseModel):
    ...
    def format_samples_for_training(self, batch):
        ...
        teacher_targets_ensemble = None
        if "teacher_delta" in batch:
            teacher_delta = batch["teacher_delta"].to(device)
            teacher_delta = teacher_delta[:, :T - 1, :]
            if teacher_delta.shape[-1] != self.max_obs_dim:
                pad_dim = self.max_obs_dim - teacher_delta.shape[-1]
                if pad_dim > 0:
                    teacher_delta = F.pad(teacher_delta, (0, pad_dim), value=0.0)
                elif pad_dim < 0:
                    teacher_delta = teacher_delta[..., :self.max_obs_dim]
            teacher_delta = teacher_delta.reshape(-1, teacher_delta.shape[-1])
            teacher_targets_ensemble = teacher_delta[data_idxes]

        return inputs_ensemble, targets_ensemble, training_target_masks, teacher_targets_ensemble
    ...
    def forward(self, batch):
        ...
        inputs, targets, masks, teacher_targets = self.format_samples_for_training(batch)
        ...
        return mean, logvar, max_logvar, min_logvar, targets, masks, teacher_targets

    def compute_nll_loss(self, mean, logvar, targets, masks):
        """
        Compute negative log-likelihood style loss (without regularization terms).
        """
    ...
    def compute_multi_step_loss(self, batch):
        ...
        Start from the real s_0, use the model-predicted next state as input,
        roll forward for H steps, and compare predicted vs. real observations
        at each step.
        """
        device = self.device
        obs = batch["obs"].to(device)      # [B, T, obs_dim]
        ...
        # Mask: real observation dimensions are 1, padded dimensions are 0.
        masks = torch.cat(
            [
                torch.ones(obs_dim, device=device),
                torch.zeros(obs_padding_dim, device=device)
        ...
        # Shape [E, B, max_obs_dim], with one copy per ensemble member.
        ...
        for t in range(H):
            # Current real next observation (t+1).
            ...
            # Action at the current step (real action).
            ...
            # Predict the delta with the model.
            ...
            # Accumulate losses and average over the H time steps later.
            total_mse = total_mse + (mse * inv_var).sum(dim=(1,2)) / masks.sum() / B
            total_var = total_var + logvar_masked.sum(dim=(1,2)) / masks.sum() / B

            # Update the predicted state with the model mean_delta (closed-loop rollout).
    ...
    def training_step(self, batch, batch_idx):
        """PyTorch Lightning training step with probabilistic loss."""
        mean, logvar, max_logvar, min_logvar, targets, masks, teacher_targets = self.forward(batch)
        hard_loss = self.compute_loss(mean, logvar, max_logvar, min_logvar, targets, masks)

        loss = hard_loss
        soft_loss = None
        if self.training and self.kd_enabled and teacher_targets is not None:
            soft_loss = self.compute_nll_loss(mean, logvar, teacher_targets, masks)
        ...
        if self.rollout_horizon > 1 and self.multi_step_coef > 0.0:
            ...
            loss = one_step_loss + self.multi_step_coef * multi_step_loss
        ...
        wandb.log(
            {
                ...
                "train_step_hard_loss": hard_loss.item(),
                "train_step_soft_loss": soft_loss.item() if soft_loss is not None else 0.0,
            },
    ...
    def validation_step(self, batch, batch_idx):
        ...
        """
        mean, logvar, max_logvar, min_logvar, targets, masks, _ = self.forward(batch)
